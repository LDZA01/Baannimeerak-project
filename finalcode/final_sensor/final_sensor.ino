...
void IRAM_ATTR detectsMovement() {
  ...
  digitalWrite(2, HIGH); //tell camera
  startTimer = true;
...
void loop(){
  while (espSerial.available() > 0){
    // get the byte data from the ESP
  ...
  ////////////////////////////////////////pir (motion) + Tell camera to take picture/////////////////////////////////////
  // Current time
  now = millis();
  if((digitalRead(led_pir) == HIGH) && (motion == false)) {
    //digitalWrite(2, HIGH); //tell camera to take pictre already done infunction
  ...
  String data = "{\"data\":{\"Humidity\":" + String(humidity) 
              ...
              + ",\"LIGHT_Val\":" + String(val_light) 
              + ",\"Sound_Val\":" + String(val_sound) 
              + ",\"Rain_val\":" + String(val_rain) 
              + "}}";  
  Serial.println(data);
